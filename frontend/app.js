...
async function fetchCategories() {
  try {
    ...
    if (data.success && data.categories) {
      ...
      logToConsole(`Loaded ${data.categories.length} categories from DB in ${data.executionTimeMs}ms`, 'success');
    }
  } catch (err) {
...
async function fetchProducts(cursor = null) {
  ...
  try {
    ...
    logToConsole(`Querying API: GET /products${cursor ? `?cursor=${cursor.slice(0, 15)}...` : ''}${currentCategory ? `&cat=${currentCategory}` : ''}`, 'query');
    ...
    if (data.success) {
      ...
      logToConsole(`Returned ${data.products.length} products. Execution Time: ${data.executionTimeMs}ms`, 'success');
      ...
      data.products.forEach((product, idx) => {
        ...
        if (nameLower.includes('simulated new')) {
          badgesHtml = '<span class="sim-badge badge-simulated-new">New Simulated</span>';
        } else if (nameLower.startsWith('updated')) {
          badgesHtml = '<span class="sim-badge badge-simulated-updated">Updated</span>';
        }
...
menuItems.forEach(item => {
  item.addEventListener('click', () => {
    ...
    tabTitle.textContent = label === 'Catalog Feed' ? 'Product Catalog Feed' : label;
...
btnNext.addEventListener('click', () => {
  if (currentNextCursor) {
    ...
    logToConsole(`Navigating to Page ${currentPage}. Appended cursor to history stack.`, 'info');
  }
});
...
btnPrev.addEventListener('click', () => {
  if (cursorStack.length > 1) {
    ...
    logToConsole(`Navigating back to Page ${currentPage}. Popped cursor from history stack.`, 'info');
  }
});
...
categorySelect.addEventListener('change', (e) => {
  ...
  cursorStack = [null];
  currentPage = 1;
  logToConsole(`Category filter changed to: "${currentCategory || 'ALL'}". Reset pagination to Page 1.`, 'info');
  fetchProducts(null);
});
...
btnAdd.addEventListener('click', async () => {
  ...
  logToConsole('Sending simulation request: Inject 50 new products at the top...', 'query');
  
  try {
    const res = await fetch(`${API_BASE}/products/simulate-add`, { method: 'POST' });
    const data = await res.json();
    
    if (data.success) {
      showToast('Added 50 new products!');
      logToConsole(`Simulated Event: Successfully added 50 mock products with created_at = NOW()`, 'success');
      logToConsole(`[Pagination Safety check] Keyset pagination filters items using 'WHERE created_at < cursor'. Since these 50 new products have timestamps newer than your cursor, they are positioned above your current viewing cursor. Your feed is 100% stable; you will not experience shifting rows or duplicate items when clicking "Next Page".`, 'info');
      
      // If we are on page 1, refresh the catalog to show the new items immediately
      if (currentPage === 1) {
        logToConsole(`You are currently on Page 1. Refreshing feed to display new additions at the top.`, 'info');
        fetchProducts(null);
      }
    } else {
...
btnUpdate.addEventListener('click', async () => {
  ...
  logToConsole('Sending simulation request: Update 50 random products...', 'query');
  
  try {
    const res = await fetch(`${API_BASE}/products/simulate-update`, { method: 'POST' });
    const data = await res.json();
    
    if (data.success) {
      showToast('Updated 50 random products!');
      logToConsole(`Simulated Event: Modified fields for 50 random products in the database.`, 'success');
      logToConsole(`[Pagination Safety check] Since we sort by created_at (which is immutable), their indexes and positions in the feed remain stable. When you navigate to them, you will see their updated properties without skipping or double-counting products.`, 'info');
